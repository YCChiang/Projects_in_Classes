...
int write_file(char *file_name, filesystem *sys)
{
    ...
    temp->position = lseek(sys->file_fd, (off_t)0, SEEK_END)+1;
    pwrite(sys->file_fd, file_content, temp->length, temp->position);
    pthread_rwlock_unlock(&sys->file_lock);
...
filesystem *open_filesystem()
{
    ...
    sys->file_fd = open(SYSTEM_FILE, O_CREAT| O_WRONLY | O_APPEND, S_IRUSR | S_IWUSR);
    load_table(sys);
...
int main()
{
    ...
    buffer = (char*)malloc(sizeof(char)*MAX_NAME_LEN);
    sprintf(buffer, "4.html");
    ret = write_file(buffer, sys);
    printf("%d\n", ret);

    buffer = (char*)malloc(sizeof(char)*MAX_NAME_LEN);
    sprintf(buffer, "1.html");
